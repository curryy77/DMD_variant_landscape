...
The project fixes key reproducibility sources: single `RANDOM_STATE=42` in the ML pipeline, group-aware validation (`StratifiedGroupKFold`), and deterministic class balancing with fixed seed. All dependencies are listed in [requirements.txt](/Users/franceballin/PycharmProjects/DMD_variant_landscape/requirements.txt), major artifacts are saved in `models/` (`model_logreg.pkl`, `model_random_forest.pkl`, `model.pkl`) and `figures/` (ROC/feature importance/confusion matrix and EDA/expression plots).
